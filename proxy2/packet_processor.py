...
#     print('final', pkt_ip.summary)
#     pkt.accept()

# nfqueue = NetfilterQueue()
# nfqueue.bind(1, nfqueue_cb)
# try:
#     print('running')
#     nfqueue.run()
...
class PacketProcessor(object):
    ...
    def _process_packet_helper(self, raw_pkt):

        try:
            pkt = sc.IP(raw_pkt.get_payload())
            # print('new pkt', pkt.summary)
        except Exception as e:
        ...
        #if src_mac == "61:01:94:3b:d2:79" or dst_mac == "61:01:94:3b:d2:79":
        #    print('Ether', pkt[sc.Ether])
        ...
        elif self._host_state.host_ip in (pkt[sc.IP].src, pkt[sc.IP].dst):
            raw_pkt.accept()
            return
        ...
        if sc.TCP in pkt:
            protocol = 'tcp'
            sport, dport = pkt[sc.TCP].sport, pkt[sc.TCP].dport
        elif sc.UDP in pkt:
            protocol = 'udp'
            sport, dport = pkt[sc.UDP].sport, pkt[sc.UDP].dport
