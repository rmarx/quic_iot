...
MODE = 'DomainNoDigit' #'Classic' # 'PortLess', 'SubnetLess8', 'SubnetLess16', 'NSLookup', 'DomainNoDigit'
    ...
    def control_match(self, pkt, interval, pkt_len, pkt_flag, count=True, during_manual=False, debug=False):
        ...
        if not during_manual:
            for control_item in self.control_candidates:
                ...
                if (np.absolute(control_item[0] - interval) <= utils.MAX_INTERVAL_ERROR 
                    ...
                    if debug:
                        print('control_match is control - new!', count, (interval, pkt_len, pkt_flag), control_item)
                    return True
        ...
        for control_item in self.control_confirmed:
            if (np.absolute(control_item[0] - interval) <= utils.MAX_INTERVAL_ERROR 
                ...
                self.last_confirmed_pkt_ts[pkt_len] = float(pkt.time)
                if debug:
                    print('control_match is control - confirmed!', PPP, count, (interval, pkt_len, int(pkt_flag)), control_item)
        ...
        if count and (not during_manual):
            if interval <= utils.MAX_REGULAR_INTERVAL:
                ...
                if debug:# or pkt_len==-230:
                    print('control_match add control candidate', PPP, str([interval, pkt_len, int(pkt_flag), float(pkt.time)]))
                    # print(self.control_candidates)
...
class Device(object):
    ...
    def new_pkt(self, pkt, pkt_id, filter_inout=None, IP_filter=[]):
        ret = 0
        # print('new_pkt', self.name, pkt_id, pkt.summary)
        ...
        if flow_id:
            if filter_inout == 'in' and flow_id in IP_filter:
                ...
                # if len(pkt) == 74:
                #     debug = True
                #     print('debug, pkt_id, flow_id', pkt_id, flow_id)
                ...
                ret = self.process_queue_long(pkt, is_control, is_burst, debug=debug)
    ...
    def simple_rule(self, pkt):
        if self.name == "SP10":
            if TCP in pkt and len(pkt) == 235-14:
                print('SP10 manual!')
                self.queue_label = 2
            else:
                self.queue_label = 0
            ...
            if TCP in pkt and len(pkt) == 267-14:
                print('WP3 manual!')
                self.queue_label = 2
            else:
                self.queue_label = 0
    ...
    def process_queue_long(self, pkt, is_control, is_burst, debug=False):
        ret = 0 
        ...
        if debug:
            print(is_control, is_burst, is_unpredictable)
        if is_unpredictable:
    ...
    def analyze_short(self):
        ...
        if self.unexpected_queue_long_pktid[0] > 80:
            self.queue_label = 2
            print('Force to %d' % (self.queue_label))
        # if self.unexpected_queue_long_pktid[0] < 80:
        #     self.queue_label = 0
            # print('Force to %d' % (self.queue_label))
...
class Predictor(object):
    ...
    def new_pkt(self, pkt, pkt_id, filter_inout=None, IP_filter=[]):
        ...
        if (IP not in pkt): #(Ether not in pkt or pkt[Ether].type == VLAN) or (IP not in pkt):
            #print('no Ether or IP', pkt.summary)
            return ret
        ...
        if sc.Padding in pkt:
            ...
            if pkt[IP].src in self.devices:
                #print(pkt[IP].src, self.devices[pkt[IP].src])
