...
port    = 8081                    # default listening port 
...
def th_web_app():
    ...
    cherrypy.tree.mount(StringGeneratorWebService(), '/install-browser', conf)
    cherrypy.tree.mount(StringGeneratorWebService(), '/launch', conf)
...
class StringGeneratorWebService(object):
	...
	def POST(self, name="test"):
		...
		# install browser from playstore 
		elif 'install-browser' in cherrypy.url():
			browser = read_json(cherrypy.request)
			assignmentId = '1234'
			#command = "./close-all.sh" + ' ' + device_id
			#run_bash(command)
			command = "../human-automation/automate.sh install " + assignmentId + ' ' + device_id  + ' ' + browser
			run_bash(command)
		
		# install browser from playstore 
		elif 'launch' in cherrypy.url():
			browser = 'firefox'
			assignmentId = '1234'
			command = "../human-automation/automate.sh launch " + assignmentId + ' ' + device_id  + ' ' + browser
			run_bash(command)
		...
		# user test start 
			...
			last_command_time = time.time()
			command = "../automation/usability-test.sh start " + assignmentId + ' ' + device_id + ' ' + session_id
			run_bash(command)
		...
		# user test end
			...
			user_test = False 
			assignmentId = read_json(cherrypy.request)
			command = "../automation/usability-test.sh stop " + assignmentId + ' ' + device_id + ' ' + session_id
			run_bash(command)
	
		# user test end
		elif 'replay' in cherrypy.url():
			assignmentId = read_json(cherrypy.request)
			command = "../automation/usability-test.sh replay " + assignmentId + ' ' + device_id
			run_bash(command)
...
if __name__ == '__main__':
	...
	# find phone whose screen is being mirrored
	find_phone()
	
	# get info about this device 
	get_device_info('phones-info.json')
