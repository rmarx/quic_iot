# ----------------------- What's New ----------------------- #
# 1. Add time constraints
# 2. Output contacted IPs
# 3. Accept IP filters
# --------------------------- END -------------------------- #
...
if __name__ == "__main__":
    ...
    if len(sys.argv) <= 3 or len(sys.argv) == 6 or len(sys.argv) == 8 or len(sys.argv) >= 10:
    ...
    if len(sys.argv) >= 7:
        start_ts = float(sys.argv[5]) if sys.argv[5] != 'None' else None
        end_ts = float(sys.argv[6]) if sys.argv[6] != 'None' else None
    if len(sys.argv) >= 9:
        filter_inout = sys.argv[7]
        if filter_inout != 'in' and filter_inout != 'out':
            print('Input format: python regular_trace_extract.py mode device_file input_file [output_file] [start_ts end_ts] [(in/out) filter_file]')
            exit()
        filter_file = sys.argv[8]
