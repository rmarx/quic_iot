...
def plot_warning(devices, non_iot_devices, output_dir, method, symlog=False, x_min=None, x_max=None):
    plt.clf()
    ...
    if symlog:
        plt.savefig(os.path.join(output_dir, 'warnings_%s_log.pdf' % (method)))
    else:
        plt.savefig(os.path.join(output_dir, 'warnings_%s.pdf' % (method)))
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 4:
        ...
        print('Input format: python plot_data_trans.py method device_file input_file [output_dir] [start_ts end_ts]')
        exit()
    ...
    plot_warning(devices, non_iot_devices, output_dir, method, symlog=False, x_min=start_ts, x_max=end_ts)
    plot_warning(devices, non_iot_devices, output_dir, method, symlog=True, x_min=start_ts, x_max=end_ts)
