...
def plot_regular_proportion(summary, output_dir):
    ...
    plt.clf()
    results = percentages['withport']['reguarl1']['count']
    x, y = sorted(results), np.arange(1, 1 + len(results)) / len(results)
    plt.plot(x, y, label='Predict1-Port', color=colors[0], linewidth = 2)

    results = percentages['noport']['reguarl1']['count']
    x, y = sorted(results), np.arange(1, 1 + len(results)) / len(results)
    plt.plot(x, y, label='Predict1-NoPort', color=colors[1], linewidth = 2)

    results = percentages['withport']['reguarl2']['count']
    x, y = sorted(results), np.arange(1, 1 + len(results)) / len(results)
    plt.plot(x, y, label='Predict2-Port', color=colors[2], linewidth = 2)

    results = percentages['noport']['reguarl2']['count']
    x, y = sorted(results), np.arange(1, 1 + len(results)) / len(results)
    plt.plot(x, y, label='Predict2-NoPort', color=colors[3], linewidth = 2)

    # results = percentages['withport']['reguarl3']['count']
    # x, y = sorted(results), np.arange(1, 1 + len(results)) / len(results)
    # plt.plot(x, y, label='Predict3-Port', color=colors[4], linewidth = 2)

    # results = percentages['noport']['reguarl3']['count']
    # x, y = sorted(results), np.arange(1, 1 + len(results)) / len(results)
    # plt.plot(x, y, label='Predict3-NoPort', color=colors[5], linewidth = 2)

    plt.ylim(0, 1)
    plt.ylabel('CDF (0-1)')
    plt.xlabel('Percentage Predictable Traffic by Flow Number (%)')
    plt.tight_layout()
    plt.grid(True)
    plt.legend()#prop={'size': 10})
    plt.savefig(os.path.join(output_dir, 'cdf_count.pdf'))


    plt.clf()
    results = percentages['withport']['reguarl1']['volume']
    x, y = sorted(results), np.arange(1, 1 + len(results)) / len(results)
    plt.plot(x, y, label='Predict1-Port', color=colors[0], linewidth = 2)

    results = percentages['noport']['reguarl1']['volume']
    x, y = sorted(results), np.arange(1, 1 + len(results)) / len(results)
    plt.plot(x, y, label='Predict1-NoPort', color=colors[1], linewidth = 2)
    ...
    # results = percentages['withport']['reguarl3']['volume']
    # x, y = sorted(results), np.arange(1, 1 + len(results)) / len(results)
    # plt.plot(x, y, label='Predict3-Port', color=colors[4], linewidth = 2)

    # results = percentages['noport']['reguarl3']['volume']
    # x, y = sorted(results), np.arange(1, 1 + len(results)) / len(results)
    # plt.plot(x, y, label='Predict3-NoPort', color=colors[5], linewidth = 2)

    plt.ylim(0, 1)
    plt.ylabel('CDF (0-1)')
    plt.xlabel('Percentage Predictable Traffic by Volume (%)')
    plt.tight_layout()
    plt.grid(True)
    plt.legend()#prop={'size': 10})
    plt.savefig(os.path.join(output_dir, 'cdf_volume.pdf'))
    ...
    results = percentages['withport']['reguarl2']['max_int']
    x, y = sorted(results), np.arange(1, 1 + len(results)) / len(results)
    plt.plot(x, y, label='Predict2-Port', color=colors[2], linewidth = 2)

    results = percentages['noport']['reguarl2']['max_int']
    x, y = sorted(results), np.arange(1, 1 + len(results)) / len(results)
    plt.plot(x, y, label='Predict2-NoPort', color=colors[3], linewidth = 2)
